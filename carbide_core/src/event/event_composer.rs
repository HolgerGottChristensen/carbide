...
use crate::event::{Button, Gesture, Input, ModifierKey, Motion, MouseButton, MouseEvent, MouseEventContext, OtherEventContext, TouchPhase, WindowEvent};
use crate::event::Event;
use crate::event::keyboard_event_handler::{KeyboardEvent, KeyboardEventContext};
...
pub struct EventComposer {
    ...
    events: Vec<(Event, Option<WindowId>)>,
    any_focus: bool,
}
...
impl EventComposer {
    pub fn delegate_events(&mut self, widgets: &mut impl AnyWidget, env: &mut Environment, text_context: &mut dyn InnerTextContext, image_context: &mut dyn InnerImageContext) -> bool {
        ...
        for (event, window_id) in events {
            ...
            match event {
                Event::Mouse(mouse_event) => {
                    ...
                    widgets.process_mouse_event(mouse_event, &mut MouseEventContext {
                        text: text_context,
                        image: image_context,
                        env,
                        is_current: &false,
                ...
                Event::Keyboard(keyboard_event) => {
                    widgets.process_keyboard_event(keyboard_event, &mut KeyboardEventContext {
                        text: text_context,
                        image: image_context,
                        env,
                        is_current: &false,
            ...
            if let Some(request) = env.focus_request.clone() {
                ...
                match event {
                    Event::Keyboard(KeyboardEvent::Press(key, modifier)) => {
                        if key == &Key::Tab {
        ...
        widgets.process_other_event(&Event::DoneProcessingEvents, &mut OtherEventContext {
            text: text_context,
            image: image_context,
            env,
        });
    ...
    pub fn get_events(&self) -> &Vec<(Event, Option<WindowId>)> {
        &self.events
    }
...
impl EventComposer {
    ...
    fn add_event(&mut self, event: Event, window_id: Option<WindowId>) {
        /*if !matches!(event, WidgetEvent::Mouse(MouseEvent::Move {..})) {
            println!("Event: {:#?}", event);
        }*/

        if let Event::Mouse(MouseEvent::Move {
            delta_xy,
            to,
            modifiers,
            ..
        }) = event
        {
            ...
            if delta_xy.x != 0.0 || delta_xy.y != 0.0 {
                ...
                if let Some((Event::Mouse(MouseEvent::Move {
        ...
        } else if let Event::Mouse(MouseEvent::Scroll {
            x: new_x,
            y: new_y,
        ...
        }) = event
        {
            ...
            if let Some((Event::Mouse(MouseEvent::Scroll {
                x,
                y,
                mouse_position,
                modifiers,
        ...
        match event {
            ...
            Input::Press(button_type) => match button_type {
                ...
                Button::Mouse(mouse_button) => {
                    ...
                    self.add_event(Event::Mouse(event.clone()), window_id);
                ...
                Button::Keyboard(key) => {
                    self.add_event(Event::Keyboard(KeyboardEvent::Press(key, modifiers)), window_id);
                    None
                }
            }
            ...
            Input::Release(button_type) => match button_type {
                Button::Mouse(mouse_button) => {
                    ...
                    self.add_event(Event::Mouse(event), window_id);
                    ...
                    if is_click {
                        if click_number == 1 {
                            self.add_event(Event::Mouse(MouseEvent::Click(mouse_button, mouse_xy, modifiers)), window_id);
                            self.last_click = Some((Instant::now(), MouseEvent::Click(mouse_button, mouse_xy, modifiers)));
                        } else {
                            self.add_event(Event::Mouse(MouseEvent::NClick(mouse_button, mouse_xy, modifiers, click_number)), window_id);
                ...
                Button::Keyboard(key) => {
                    ...
                    self.add_event(Event::Keyboard(event), window_id);
            ...
            Input::Resize(w, h) => {
                ...
                self.add_event(Event::Window(event), window_id);
            ...
            Input::Motion(motion) => {
                match motion {
                    Motion::MouseCursor { x, y } => {
                        ...
                        self.add_event(Event::Mouse(move_event), window_id);
                            ...
                            for (button, evt) in self.pressed_buttons.iter() {
                                match evt {
                                    MouseEvent::Press(_, location, _) => {
                                        ...
                                        events.push(Event::Mouse(drag_event));
                                    }
                                    _ => {}
                                }
                            }
            ...
            Input::Focus(focused) if focused == true => {
                self.add_event(Event::Window(WindowEvent::Focus), window_id);
                Some(WindowEvent::Focus)
                //ui.needs_redraw()
            }
            Input::Focus(_focused) => {
                self.add_event(Event::Window(WindowEvent::UnFocus), window_id);
                None
            }
            ...
            Input::Custom(event) => {
                self.add_event(Event::Custom(event), window_id);
                None
            }
            Input::CloseRequested => {
                self.add_event(Event::Window(WindowEvent::CloseRequested), window_id);
                None
            }
            Input::Gesture(Gesture::Scale(scale, phase)) => {
                self.add_event(Event::Mouse(MouseEvent::Scale(scale, mouse_xy, phase)), window_id);
                None
            }
            Input::Gesture(Gesture::Rotate(delta, phase)) => {
                self.add_event(Event::Mouse(MouseEvent::Rotation(delta, mouse_xy, phase)), window_id);
                None
            }
            Input::Gesture(Gesture::SmartScale) => {
                self.add_event(Event::Mouse(MouseEvent::SmartScale(mouse_xy)), window_id);
                None
            }
            ...
            Input::Ime(ime) => {
                self.add_event(Event::Keyboard(KeyboardEvent::Ime(ime)), window_id);
                None
            }
        }
    }
