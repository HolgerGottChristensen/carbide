...
"Hello World", but it'll introduce most of the key concepts behind carbide.
...
cargo to create a new project. I've called the project carbide_hw for hello
world, you can use any name. Type at the command line:

```txt
cargo new carbide_hw --bin
```
...
```txt
   Compiling carbide_hw v0.1.0
   (file:///Users/declan/Projects/rust/carbide_hw)
   Finished dev [unoptimized + debuginfo] target(s) in 0.34 secs
   Running `target/debug/carbide_hw`
   Hello, world!
```
...
## Setting up Cargo for carbide

To use carbide in your application, you need to tell your program to use it,
...
To allow your programme to use carbide, you need to add a reference to the
carbide crate, with:

```ignore
#[macro_use] extern crate carbide;
...
```txt
error[E0463]: can't find crate for "carbide"
...
Add `carbide = { version = "0.55.0", features = ["glium", "winit"] }` to the
...
carbide is designed as an IMGUI that can use a number of backends. It was
...
handled by conditional compilation in the carbide code. For simplicity, I'm
...
developer of carbide. Glium has a very good, gentle
[introduction](http://glium.github.io/glium/book/) to OpenGL - enough to
convince me that I really want carbide to do the work for me!

carbide makes Glium available through an export. Put the following line into
the code, near the top, to include it:

```ignore
use carbide::backend::glium::glium::{self, Surface};
```
...
Now create the UI itself. carbide has a builder that contains and looks after
the UI for the user.

```ignore
let mut ui = carbide::UiBuilder::new([WIDTH as f64, HEIGHT as f64]).build();
```


carbide can use graphics. It stores these in a map. The system needs the map,
even though it doesn't contain anything at this time, so create it:

```ignore
let image_map = carbide::image::Map::<glium::texture::Texture2d>::new();
```

Finally, carbide needs to render its UI. It uses a renderer to do this, so
create one:

```ignore
let mut renderer = carbide::backend::glium::Renderer::new(&display).unwrap();
```

As an Immediate Mode GUI, carbide sits in the main loop of the program, drawing
...
`widget_ids!` is, as its name suggests, a macro provided by carbide to create
widget ids. carbide's primary data structure contains all the widgets, and uses
...
carbide::widget::Text<'_>' in the current scope
  ...
  --> src/lib:89:18`
   ...
   = note: the following trait is implemented but not in scope, perhaps add a 'use' for it:`
           candidate #1: 'use carbide::Positionable;'
```


`middle_of` is a carbide trait, of the type `Positionable` and color is also a
trait of type 'Colorable'. To ensure that the program can find the trait, we
need to add to the `use carbide` line, as suggested in the error message. We
also need to define `widget' and the type of Widget, so add the following near
the top of the programme:
```ignore
use carbide::{widget, Positionable, Colorable, Widget};
```

There's one more thing we need to do. carbide needs to load fonts before it can
...
First, let's add event handling for carbide. Putting the following at the
beginning of the event for loop will take care of UI events:

```ignore
if let Some(event) = carbide::backend::winit::convert_event(
    event.clone(),
    &display
) {
...
`ui.handle_event()` is the business end of carbide - it takes events off the
...
Then there's some straightforward event boilerplate that most carbide apps have
...
`carbide_hw` is now taking 0% of available CPU. Much better!


This chapter has introduced the basic elements of a carbide application: the
...
The code produced in this chapter is available on the carbide Github site
within the examples.
