...
impl<T: StateContract> TState<Option<T>> {
    ...
    pub fn is_some(&self) -> RState<bool> {
        Map1::read_map(self.clone(), |t: &Option<T>| t.is_some())
    }
    ...
    pub fn is_none(&self) -> RState<bool> {
        Map1::read_map(self.clone(), |t: &Option<T>| t.is_none())
    }
    ...
    pub fn ok_or<E: StateContract>(&self, err: impl Into<TState<E>>) -> TState<Result<T, E>> {
        Map2::map(
            self.clone(),
            err.into(),
            |one: &Option<T>, other: &E| one.clone().ok_or_else(|| other.clone()),
            |new: Result<T, E>, one: &Option<T>, other: &E| match new {
                Ok(a) => (Some(Some(a)), None),
                Err(e) => (Some(None), Some(e)),
            },
        )
    }
    ...
    pub fn unwrap_or(&self, default: impl Into<TState<T>>) -> TState<T> {
        Map2::map(
            self.clone(),
            default.into(),
            |state: &Option<T>, default: &T| state.clone().unwrap_or(default.clone()),
            |new, _, _| (Some(Some(new)), None),
        )
    }

    /// Zip the state and the other state to a tuple
    pub fn zip<U: StateContract>(
        &self,
        other: impl Into<TState<Option<U>>>,
    ) -> TState<Option<(T, U)>> {
        Map2::map(
            self.clone(),
            other.into(),
            |some: &Option<T>, other: &Option<U>| some.clone().zip(other.clone()),
            |new, some_old, other_old| match new {
                None => (Some(None), Some(None)),
                Some((a, b)) => (Some(Some(a)), Some(Some(b))),
            },
        )
    }
}
...
impl<T: StateContract, U: StateContract> TState<Option<(T, U)>> {
    ...
    pub fn unzip(&self) -> (RState<Option<T>>, RState<Option<U>>) {
        let a = Map1::read_map(self.clone(), |a: &Option<(T, U)>| match a {
            None => None,
            Some((b, _)) => Some(b.clone()),
        });
        let b = Map1::read_map(self.clone(), |a: &Option<(T, U)>| match a {
            None => None,
...
impl<T: StateContract + Default + 'static> TState<Option<T>> {
    ...
    pub fn unwrap_or_default(&self) -> TState<T> {
        Map1::map(
            self.clone(),
            |val: &Option<T>| val.clone().unwrap_or_default(),
            |new, _| Some(Some(new)),
        )
    }
}
