...
impl Graph {
    ...
    pub fn get_connected_edges_iter(&self, node_id: usize) -> impl Iterator<Item = &Edge> + '_ {
        self.get_incoming_edges_iter(node_id)
    ...
    pub fn get_incoming_edges_iter(&self, node_id: usize) -> impl Iterator<Item = &Edge> + '_ {
        let node = self.get_node(node_id);

        node.get_incoming_edges_iter()
    ...
    pub fn get_outgoing_edges_iter(&self, node_id: usize) -> impl Iterator<Item = &Edge> + '_ {
        let node = self.get_node(node_id);

        node.get_outgoing_edges_iter()
            .map(move |edge_id: &usize| -> &Edge {
                let edge = self.get_edge(*edge_id);
                edge
            })
    ...
    pub fn guides_and_position(&mut self, position: Position, ignore_node_id: usize) -> Position {
        ...
        for neighbour_id in 0..self.nodes.len() {
            ...
            if neighbour_id == ignore_node_id {
                continue;
            }
    ...
    fn line_from_guide(guide: &Guide) -> Option<Line> {
        match guide {
            Guide::Vertical(x) => Some(Line::new(Position::new(*x, 0.0), Position::new(*x, 30.0))),
            Guide::Horizontal(y) => {
                Some(Line::new(Position::new(0.0, *y), Position::new(30.0, *y)))
            }
            Guide::Directional(line) => Some(*line),
            Guide::Point(_) => None,
        }
    }
    ...
    pub fn calculate_lines(&mut self) {
        for node_id in 0..self.nodes.len() {
            ...
            for neighbor in self.get_outgoing_edges_iter(node_id) {
                ...
                lines.push((
                    neighbor.id,
                    Line::new(start_node.position, end_node.position),
                    true,
                    neighbor.offset,
                    neighbor.width,
                ));
