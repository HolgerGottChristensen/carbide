...
use carbide_core::state::{
    FocusState, Map1, Map3, MapOwnedState, State, StateExt, StateKey, StringState,
};
...
impl CheckBox {
    pub fn new<S: Into<StringState>, L: Into<CheckBoxState>>(
        ...
        let mut plain = PlainCheckBox::new(label, checked.into()).delegate(Self::delegate);
        plain
    }
    ...
    fn delegate(_: FocusState, checked: CheckBoxState) -> Box<dyn Widget> {
        ...
        let checked_color = Map3::read_map(
            ...
            accent,
            secondary,
            |checked: &CheckBoxValue, accent: &Color, secondary: &Color| match *checked {
        ...
        let checked_true = checked
            .map(|check: &CheckBoxValue| *check == CheckBoxValue::True)
            .ignore_writes();

        let checked_intermediate = checked
            .map(|check: &CheckBoxValue| *check == CheckBoxValue::Intermediate)
            .ignore_writes();
