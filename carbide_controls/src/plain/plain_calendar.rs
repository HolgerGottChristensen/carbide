...
use carbide::render::matrix::num_traits::FromPrimitive;
use carbide::state::AnyReadState;
use carbide_core::CommonWidgetImpl;
use carbide_core::draw::{Dimension, Position, color::TRANSPARENT};
use carbide_core::environment::EnvironmentColor;
use carbide_core::state::{AnyState, LocalState, Map1, Map2, ReadState, ReadStateExtNew, State, StateExtNew, Map3};
use carbide_core::widget::{AnyWidget, CommonWidget, Rectangle, Text, Widget, WidgetExt, WidgetId, ZStack, ForEach, HStack, Image, Spacer, VGrid, VGridColumn, VStack};
...
impl<K> PlainCalendarTitleDelegate for K where K: Fn(Box<dyn AnyState<T=Month>>, Box<dyn AnyState<T=i32>>, Selection) -> Box<dyn AnyWidget> + Clone + 'static {
    fn call(&self, month: impl State<T=Month>, year: impl State<T=i32>, selection: Selection) -> Box<dyn AnyWidget> {
        self(month.as_dyn(), year.as_dyn(), selection)
    }
}

type DefaultPlainCalendarTitleDelegate = fn(Box<dyn AnyState<T=Month>>, Box<dyn AnyState<T=i32>>, Selection) -> Box<dyn AnyWidget>;
...
impl Into<Selection> for LocalState<Option<NaiveDate>> {
    fn into(self) -> Selection {
        Selection::Single(self.as_dyn())
    }
}

impl Into<Selection> for LocalState<HashSet<NaiveDate>> {
    fn into(self) -> Selection {
        Selection::Multi(self.as_dyn())
    }
}

impl Into<Selection> for LocalState<Option<RangeInclusive<NaiveDate>>> {
    fn into(self) -> Selection {
        Selection::Range(self.as_dyn())
    }
}
...
impl PlainCalendar<DefaultPlainCalendarHeaderDelegate, DefaultPlainCalendarItemDelegate, DefaultPlainCalendarHiddenDelegate, DefaultPlainCalendarTitleDelegate> {
    pub fn new<S: Into<Selection>>(selection: S) -> PlainCalendar<DefaultPlainCalendarHeaderDelegate, DefaultPlainCalendarItemDelegate, DefaultPlainCalendarHiddenDelegate, DefaultPlainCalendarTitleDelegate> {
    ...
    fn default_item_delegate(selected: impl ReadState<T=Selected>, hovered: impl ReadState<T=bool>, pressed: impl ReadState<T=bool>, date: impl ReadState<T=NaiveDate>) -> Box<dyn AnyWidget> {
    ...
    fn default_title_delegate(month: impl State<T=Month>, year: impl State<T=i32>, selection: Selection) -> Box<dyn AnyWidget> {
        HStack::new((
        ...
        let grid = VGrid::new((
            ...
            ForEach::new_read(days_in_month, move |day: Box<dyn AnyState<T=u32>>, _| {
                ...
                let selected = match selection2.clone() {
                    Selection::Single(s) => Map2::read_map(date.clone(), s, |d, s| {
                        if Some(d) == s.as_ref() {
                            Selected::Start
                        } else {
                            Selected::None
                    ...
                    Selection::Multi(m) => Map2::read_map(date.clone(), m, |d, s| {
                        if s.contains(d) {
                            Selected::Start
                        } else {
                            Selected::None
                ...
                PlainButton::new(move |_, _| {
                    ...
                    match s2.clone() {
                        Selection::Single(mut s) => {
                            *s.value_mut() = Some(date);
                        }
                        Selection::Multi(mut m) => {
