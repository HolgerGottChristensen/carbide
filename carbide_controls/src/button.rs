...
use carbide_core::state::{AnyReadState, IntoReadState, IntoState, Map2, Map3, ReadState, TState};
...
impl Button {
    ...
    pub fn new<L: IntoReadState<String>, A: Action + Clone + 'static>(label: L, action: A) -> PlainButton<TState<Focus>, A, ButtonDelegate<L::Output, bool>> {
...
impl<L: ReadState<T=String>, P: ReadState<T=bool>> PlainButtonDelegate for ButtonDelegate<L, P> {
    fn call(&self, focus: Box<dyn AnyReadState<T=Focus>>, hovered: Box<dyn AnyReadState<T=bool>>, pressed: Box<dyn AnyReadState<T=bool>>) -> Box<dyn Widget> {
        ...
        let background_color = Map3::read_map(base_color, pressed, hovered, |col, pressed, hovered| {
            if *pressed {
