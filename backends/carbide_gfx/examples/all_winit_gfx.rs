...
//! `carbide_core::render::Primitives` to the screen.

#![allow(unused_variables)]

extern crate carbide_core;
extern crate carbide_example_shared;
extern crate carbide_gfx;
...
// the winit <-> carbide conversion functions.
...
// functions.
impl<'a> carbide_winit::WinitWindow for WindowRef<'a> {
...
fn main() {
    ...
    let builder = glutin::WindowBuilder::new()
        .with_title("carbide with GFX and Glutin")
    ...
    let (context, mut device, mut factory, rtv, _) = gfx_window_glutin::init::<
        carbide_gfx::ColorFormat,
        DepthFormat,
    >(builder, context, &events_loop)
    .unwrap();
    let mut encoder: gfx::Encoder<_, _> = factory.create_command_buffer().into();

    let mut renderer = carbide_gfx::Renderer::new(
        &mut factory,
        &rtv,
    ...
    let mut image_map = carbide_core::image::ImageMap::new();
    let rust_logo = image_map.insert(load_rust_logo::<carbide_gfx::ColorFormat, _, _>(
        &mut factory,
    ));

    // Demonstration app state that we'll control with our carbide GUI.
    ...
    'main: loop {
        ...
        events_loop.poll_events(|event| {
            // Convert winit event to carbide event, requires carbide to be built with the `winit` feature
