...
use super::svgtree::{AttributeId, ElementId, SvgNode};
use crate::*;
...
pub(crate) fn convert(
    ...
    let paint = match node.tag_name().unwrap() {
        ElementId::LinearGradient => convert_linear(node, state),
        ElementId::RadialGradient => convert_radial(node, state),
        ElementId::Pattern => convert_pattern(node, state, cache),
        _ => unreachable!(),
    };
...
fn convert_linear(node: SvgNode, state: &converter::State) -> Option<ServerOrColor> {
    ...
    let units = convert_units(node, AttributeId::GradientUnits, Units::ObjectBoundingBox);
    let transform = node.resolve_transform(AttributeId::GradientTransform, state);

    let gradient = LinearGradient {
        x1: resolve_number(node, AttributeId::X1, units, state, Length::zero()),
        y1: resolve_number(node, AttributeId::Y1, units, state, Length::zero()),
        x2: resolve_number(
            node,
            AttributeId::X2,
            units,
        ...
        y2: resolve_number(node, AttributeId::Y2, units, state, Length::zero()),
        base: BaseGradient {
            id,
            units,
            transform,
...
fn convert_radial(node: SvgNode, state: &converter::State) -> Option<ServerOrColor> {
    ...
    let units = convert_units(node, AttributeId::GradientUnits, Units::ObjectBoundingBox);
    let r = resolve_number(node, AttributeId::R, units, state, Length::new(50.0, Unit::Percent));
    let fr = resolve_number(node, AttributeId::Fr, units, state, Length::zero());
    ...
    let fx = resolve_number(node, AttributeId::Fx, units, state, Length::new_number(cx as f64));
    let fy = resolve_number(node, AttributeId::Fy, units, state, Length::new_number(cy as f64));
    let transform = node.resolve_transform(AttributeId::GradientTransform, state);
...
fn convert_pattern(
    ...
    let view_box = {
        let n1 = resolve_attr(node, AttributeId::ViewBox);
        let n2 = resolve_attr(node, AttributeId::PreserveAspectRatio);
        n1.parse_viewbox().map(|vb| ViewBox {
            rect: vb,
            aspect: n2.attribute(AttributeId::PreserveAspectRatio).unwrap_or_default(),
        })
    };

    let units = convert_units(node, AttributeId::PatternUnits, Units::ObjectBoundingBox);
    let content_units = convert_units(node, AttributeId::PatternContentUnits, Units::UserSpaceOnUse);

    let transform = node.resolve_transform(AttributeId::PatternTransform, state);

    let rect = NonZeroRect::from_xywh(
        resolve_number(node, AttributeId::X, units, state, Length::zero()),
        resolve_number(node, AttributeId::Y, units, state, Length::zero()),
        resolve_number(node, AttributeId::Width, units, state, Length::zero()),
        resolve_number(node, AttributeId::Height, units, state, Length::zero()),
    );
...
fn convert_spread_method(node: SvgNode) -> SpreadMethod {
    let node = resolve_attr(node, AttributeId::SpreadMethod);
    node.attribute(AttributeId::SpreadMethod).unwrap_or_default()
}

pub(crate) fn convert_units(node: SvgNode, name: AttributeId, def: Units) -> Units {
...
fn find_gradient_with_stops<'a, 'input: 'a>(
    ...
    for link in node.href_iter() {
        ...
        if link.children().any(|n| n.tag_name() == Some(ElementId::Stop)) {
            return Some(link);
        }
    }
...
fn find_pattern_with_children<'a, 'input: 'a>(
    ...
    for link in node.href_iter() {
        if link.tag_name() != Some(ElementId::Pattern) {
            log::warn!(
...
fn convert_stops(grad: SvgNode) -> Vec<Stop> {
...

    {
        ...
        for stop in grad.children() {
            if stop.tag_name() != Some(ElementId::Stop) {
                log::warn!("Invalid gradient child: '{:?}'.", stop.tag_name().unwrap());
                continue;
            }

            // `number` can be either a number or a percentage.
            let offset = stop.attribute(AttributeId::Offset).unwrap_or(prev_offset);
            ...
            let (color, opacity) = match stop.attribute(AttributeId::StopColor) {
                Some("currentColor") => stop
                    .find_attribute(AttributeId::Color)
            ...
            let stop_opacity = stop
                .attribute::<Opacity>(AttributeId::StopOpacity)
...
fn resolve_attr<'a, 'input: 'a>(node: SvgNode<'a, 'input>, name: AttributeId) -> SvgNode<'a, 'input> {
    if node.has_attribute(name) {
        return node;
    }

    match node.tag_name().unwrap() {
        ElementId::LinearGradient => resolve_lg_attr(node, name),
        ElementId::RadialGradient => resolve_rg_attr(node, name),
        ElementId::Pattern => resolve_pattern_attr(node, name),
        ElementId::Filter => resolve_filter_attr(node, name),
        _ => node,
    }
}

fn resolve_lg_attr<'a, 'input: 'a>(node: SvgNode<'a, 'input>, name: AttributeId) -> SvgNode<'a, 'input> {
    for link in node.href_iter() {
        ...
        match (name, tag_name) {
            ...
            // ref element with the same type.
              (AttributeId::X1, ElementId::LinearGradient)
            | (AttributeId::Y1, ElementId::LinearGradient)
            | (AttributeId::X2, ElementId::LinearGradient)
            | (AttributeId::Y2, ElementId::LinearGradient)
            // Other attributes can be resolved
            // from any kind of gradient.
            | (AttributeId::GradientUnits, ElementId::LinearGradient)
            | (AttributeId::GradientUnits, ElementId::RadialGradient)
            | (AttributeId::SpreadMethod, ElementId::LinearGradient)
            | (AttributeId::SpreadMethod, ElementId::RadialGradient)
            | (AttributeId::GradientTransform, ElementId::LinearGradient)
            | (AttributeId::GradientTransform, ElementId::RadialGradient) => {
...
fn resolve_rg_attr<'a, 'input>(node: SvgNode<'a, 'input>, name: AttributeId) -> SvgNode<'a, 'input> {
    for link in node.href_iter() {
        let tag_name = match link.tag_name() {
            Some(v) => v,
            None => return node,
        ...
        match (name, tag_name) {
            // Coordinates can be resolved only from
            // ref element with the same type.
              (AttributeId::Cx, ElementId::RadialGradient)
            | (AttributeId::Cy, ElementId::RadialGradient)
            | (AttributeId::R,  ElementId::RadialGradient)
            | (AttributeId::Fx, ElementId::RadialGradient)
            | (AttributeId::Fy, ElementId::RadialGradient)
            // Other attributes can be resolved
            // from any kind of gradient.
            | (AttributeId::GradientUnits, ElementId::LinearGradient)
            | (AttributeId::GradientUnits, ElementId::RadialGradient)
...
fn resolve_filter_attr<'a, 'input: 'a>(node: SvgNode<'a, 'input>, aid: AttributeId) -> SvgNode<'a, 'input> {
    for link in node.href_iter() {
        let tag_name = match link.tag_name() {
            Some(v) => v,
            None => return node,
...
impl Paint {
    fn to_user_coordinates(&mut self, bbox: Rect, cache: &mut Cache) -> Option<()> {
        ...
        match self {
            Paint::CurrentColor => {} // unreachable
...
impl Paint {
    ...
    pub(crate) fn units(&self) -> Units {
        match self {
            ...
            Paint::CurrentColor => Units::UserSpaceOnUse,
        }
    }
