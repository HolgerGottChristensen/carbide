...
pub(crate) fn convert(
    ...
    if state.parent_clip_path.is_some() && child.tag_name() == Some(ElementId::Symbol) {
    ...
    let mut orig_ts = node.resolve_transform(AttributeId::Transform, state);
    let mut new_ts = Transform::default();

    {
        let x = node.convert_user_length(AttributeId::X, &use_state, Length::zero());
        let y = node.convert_user_length(AttributeId::Y, &use_state, Length::zero());
        new_ts = new_ts.pre_translate(x, y);
    }

    let linked_to_symbol = child.tag_name() == Some(ElementId::Symbol);
    ...
    if linked_to_symbol {
        ...
        use_state.view_box = {
            ...
            let width = if node.has_attribute(AttributeId::Width) {
                node.convert_user_length(AttributeId::Width, &use_state, def)
            } else {
                use_state.view_box.width()
            };

            let height = if node.has_attribute(AttributeId::Height) {
                node.convert_user_length(AttributeId::Height, &use_state, def)
            } else {
    ...
    if linked_to_symbol {
        ...
        let linked_to_svg = child.tag_name() == Some(ElementId::Svg);
        if linked_to_svg {
            ...
            if node.has_attribute(AttributeId::Width) {
                use_state.use_size.0 = Some(node.convert_user_length(AttributeId::Width, &use_state, def));
            }
            if node.has_attribute(AttributeId::Height) {
                use_state.use_size.1 = Some(node.convert_user_length(AttributeId::Height, &use_state, def));
            }
...
pub(crate) fn convert_svg(
    ...
    let x = node.convert_user_length(AttributeId::X, state, Length::zero());
    let y = node.convert_user_length(AttributeId::Y, state, Length::zero());
    new_ts = new_ts.pre_translate(x, y);
...
fn get_clip_rect(
    ...
    if matches!(
        symbol_node.attribute(AttributeId::Overflow),
    ...
    if use_node.tag_name() == Some(ElementId::Svg) {
        ...
        if state.use_size.0.is_none() && state.use_size.1.is_none() {
            if !(use_node.has_attribute(AttributeId::Width) && use_node.has_attribute(AttributeId::Height)) {
                return None;
            }
        }
    }
    ...
    let (x, y, mut w, mut h) = {
        let x = use_node.convert_user_length(AttributeId::X, state, Length::zero());
        let y = use_node.convert_user_length(AttributeId::Y, state, Length::zero());
...
fn use_node_size(node: SvgNode, state: &converter::State) -> (f32, f32) {
    let def = Length::new(100.0, LengthUnit::Percent);
    let w = node.convert_user_length(AttributeId::Width, state, def);
    let h = node.convert_user_length(AttributeId::Height, state, def);
    (w, h)
}
...
fn viewbox_transform(
    ...
    if node.tag_name() == Some(ElementId::Svg) {
        // If attributes `width` and/or `height` are provided on the `use` element,
        // then these values will override the corresponding attributes
        // on the `svg` in the generated tree.
        w = state.use_size.0.unwrap_or(w);
