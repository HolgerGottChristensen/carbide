...
impl<'a, 'input: 'a> SvgNode<'a, 'input> {
    ...
    pub fn try_attribute<T: FromValue<'a, 'input>>(&self, aid: AttributeId) -> Option<T> {
        let value = self
            .attributes()
            .iter()
            .find(|a| a.name == aid)
    ...
    #[inline]
    fn node_attribute(&self, aid: AttributeId) -> Option<SvgNode<'a, 'input>> {
    ...
    fn find_attribute_impl(&self, aid: AttributeId) -> Option<SvgNode<'a, 'input>> {
    ...
    fn is_presentation(&self) -> bool {
        matches!(
            ...
            AttributeId::AlignmentBaseline
                ...
                | AttributeId::BackgroundColor // non-standard SVG attribute
                | AttributeId::ClipPath
                ...
                | AttributeId::FontOpticalSizing // technically not presentation
                ...
                | AttributeId::FontVariationSettings
...
pub trait FromValue<'a, 'input: 'a>: Sized {
    ...
    fn parse(node: SvgNode<'a, 'input>, aid: AttributeId, value: &'a str) -> Option<Self>;
}

impl<'a, 'input: 'a> FromValue<'a, 'input> for &'a str {
    fn parse(_: SvgNode<'a, 'input>, _: AttributeId, value: &'a str) -> Option<Self> {
        Some(value)
    }
}
...
impl<'a, 'input: 'a> FromValue<'a, 'input> for svgtypes::Paint<'a> {
    fn parse(_: SvgNode, _: AttributeId, value: &'a str) -> Option<Self> {
        Self::from_str(value).ok()
    }
}
...
impl<'a, 'input: 'a> FromValue<'a, 'input> for SvgNode<'a, 'input> {
    fn parse(node: SvgNode<'a, 'input>, aid: AttributeId, value: &str) -> Option<Self> {
        let id = if aid == AttributeId::Href {
            svgtypes::IRI::from_str(value).ok().map(|v| v.0)
        } else {
            svgtypes::FuncIRI::from_str(value).ok().map(|v| v.0)
