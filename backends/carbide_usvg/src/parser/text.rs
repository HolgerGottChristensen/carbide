...
pub(crate) fn convert(
    ...
    let rendering_mode: TextRendering = text_node
        .find_attribute(AttributeId::TextRendering)
...
fn collect_text_chunks_impl(
    ...
    for child in parent.children() {
        if child.is_element() {
            if child.tag_name() == Some(ElementId::TextPath) {
                if parent.tag_name() != Some(ElementId::Text) {
        ...
        let anchor = parent.find_attribute(AttributeId::TextAnchor).unwrap_or_default();
        ...
        let raw_paint_order: svgtypes::PaintOrder =
            parent.find_attribute(AttributeId::PaintOrder).unwrap_or_default();
        let paint_order = super::converter::svg_paint_order_to_usvg(raw_paint_order);

        let mut dominant_baseline = parent
            .find_attribute(AttributeId::DominantBaseline)
            .unwrap_or_default();
        ...
        if parent.resolve_length(AttributeId::Kerning, state, -1.0) == 0.0 {
            apply_kerning = false;
        } else if parent.find_attribute::<&str>(AttributeId::FontKerning) == Some("none") {
            apply_kerning = false;
        }

        let font_optical_sizing = match parent.find_attribute::<&str>(AttributeId::FontOpticalSizing) {
            Some("none") => crate::FontOpticalSizing::None,
            _ => crate::FontOpticalSizing::Auto, // "auto" or missing (browser default)
        };

        let mut text_length =
            parent.try_convert_length(AttributeId::TextLength, Units::UserSpaceOnUse, state);
        ...
        let visibility: Visibility = parent.find_attribute(AttributeId::Visibility).unwrap_or_default();
        ...
        let span = TextSpan {
            ...
            font,
            font_size,
            small_caps: parent.find_attribute::<&str>(AttributeId::FontVariant) == Some("small-caps"),
            apply_kerning,
            font_optical_sizing,
            ...
            alignment_baseline: parent
                .find_attribute(AttributeId::AlignmentBaseline)
                .unwrap_or_default(),
            baseline_shift: convert_baseline_shift(parent, state),
            letter_spacing: parent.resolve_length(AttributeId::LetterSpacing, state, 0.0),
            word_spacing: parent.resolve_length(AttributeId::WordSpacing, state, 0.0),
            text_length,
            length_adjust: parent.find_attribute(AttributeId::LengthAdjust).unwrap_or_default(),
        };
...
fn resolve_text_flow(node: SvgNode, state: &converter::State) -> Option<TextFlow> {
    let linked_node = node.attribute::<SvgNode>(AttributeId::Href)?;
    let path = super::shapes::convert(linked_node, state)?;

    // The reference path's transform needs to be applied
    let transform = linked_node.resolve_transform(AttributeId::Transform, state);
    ...
    let start_offset: Length = node.attribute(AttributeId::StartOffset).unwrap_or_default();
    let start_offset = if start_offset.unit == LengthUnit::Percent {
        ...
        node.resolve_length(AttributeId::StartOffset, state, 0.0)
    };
...
fn convert_font(node: SvgNode, state: &converter::State) -> Font {
    let style: FontStyle = node.find_attribute(AttributeId::FontStyle).unwrap_or_default();
    ...
    let font_families = if let Some(n) = node.ancestors().find(|n| n.has_attribute(AttributeId::FontFamily))
    {
        n.attribute(AttributeId::FontFamily).unwrap_or("")
    } else {
        ""
    };
...
fn parse_font_variation_settings(node: SvgNode) -> Vec<FontVariation> {
    let value = if let Some(n) = node
        .ancestors()
        .find(|n| n.has_attribute(AttributeId::FontVariationSettings))
    {
        let v = n.attribute(AttributeId::FontVariationSettings).unwrap_or("");
...
fn conv_font_stretch(node: SvgNode) -> FontStretch {
    if let Some(n) = node.ancestors().find(|n| n.has_attribute(AttributeId::FontStretch)) {
        match n.attribute(AttributeId::FontStretch).unwrap_or("") {
...
fn resolve_font_weight(node: SvgNode) -> u16 {
    ...
    for n in nodes.iter().rev().skip(1) {
        // skip Root
        weight = match n.attribute(AttributeId::FontWeight).unwrap_or("") {
            "normal" => 400,
            "bold" => 700,
            "100" => 100,
            "200" => 200,
...
fn resolve_positions_list(text_node: SvgNode, state: &converter::State) -> Vec<CharacterPosition> {
    ...
    for child in text_node.descendants() {
        if child.is_element() {
            ...
            if !matches!(child.tag_name(), Some(ElementId::Text) | Some(ElementId::Tspan)) {
                continue;
            }
            ...
            push_list!(AttributeId::X, x);
            push_list!(AttributeId::Y, y);
            push_list!(AttributeId::Dx, dx);
            push_list!(AttributeId::Dy, dy);
...
fn resolve_rotate_list(text_node: SvgNode) -> Vec<f32> {
    ...
    for child in text_node.descendants() {
        if child.is_element() {
            if let Some(rotate) = child.attribute::<Vec<f32>>(AttributeId::Rotate) {
...
fn resolve_decoration(
    ...
    fn find_decoration(node: SvgNode, value: &str) -> bool {
        if let Some(str_value) = node.attribute::<&str>(AttributeId::TextDecoration) {
    ...
    let mut gen_style = |text_decoration: &str| {
        ...
        for node in tspan.ancestors() {
            if find_decoration(node, text_decoration) || node.tag_name() == Some(ElementId::Text) {
...
fn convert_baseline_shift(node: SvgNode, state: &converter::State) -> Vec<BaselineShift> {
    ...
    let nodes: Vec<_> = node
        .ancestors()
        .take_while(|n| n.tag_name() != Some(ElementId::Text))
        .collect();
    for n in nodes {
        if let Some(len) = n.try_attribute::<Length>(AttributeId::BaselineShift) {
        ...
        } else if let Some(s) = n.attribute(AttributeId::BaselineShift) {
            match s {
...
fn convert_writing_mode(text_node: SvgNode) -> WritingMode {
    if let Some(n) = text_node
        .ancestors()
        .find(|n| n.has_attribute(AttributeId::WritingMode))
    {
        match n.attribute(AttributeId::WritingMode).unwrap_or("lr-tb") {
