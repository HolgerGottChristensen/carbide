...
use super::svgtree::{self, AttributeId, ElementId, FromValue, SvgNode};
...
impl<'a, 'input: 'a> SvgNode<'a, 'input> {
    ...
    pub fn convert_user_length(&self, aid: AttributeId, state: &State, def: Length) -> f32 {
    ...
    pub fn resolve_length(&self, aid: AttributeId, state: &State, def: f32) -> f32 {
        debug_assert!(
    ...
    pub fn has_valid_transform(&self, aid: AttributeId) -> bool {
...
pub fn convert_doc(svg_doc: &svgtree::Document, opt: &Options) -> Result<Tree, Error> {
    ...
    let view_box = ViewBox {
        ...
        aspect: svg.attribute(AttributeId::PreserveAspectRatio).unwrap_or_default(),
    };

    let background_color = svg
        .attribute::<&str>(AttributeId::BackgroundColor)
...
fn resolve_svg_size(svg: &SvgNode, opt: &Options) -> (Result<Size, Error>, bool) {
    ...
    let mut width: Length = svg.attribute(AttributeId::Width).unwrap_or(def);
    let mut height: Length = svg.attribute(AttributeId::Height).unwrap_or(def);
    ...
    let size = if let Some(vbox) = view_box {
        ...
        let w = if width.unit == Unit::Percent {
            ...
            svg.convert_user_length(AttributeId::Width, &state, def)
        };

        let h = if height.unit == Unit::Percent {
            vbox.height() * (height.number as f32 / 100.0)
        } else {
            svg.convert_user_length(AttributeId::Height, &state, def)
        };
...
pub(crate) fn convert_element(node: SvgNode, state: &State, cache: &mut Cache, parent: &mut Group) {
    ...
    if !tag_name.is_graphic() && !matches!(tag_name, ElementId::G | ElementId::Switch | ElementId::Svg) {
        return;
    }
    ...
    if tag_name == ElementId::Use {
        super::use_node::convert(node, state, cache, parent);
        return;
    }

    if tag_name == ElementId::Switch {
...
fn convert_clip_path_elements_impl(
    ...
    match tag_name {
        ElementId::Rect | ElementId::Circle | ElementId::Ellipse | ElementId::Polyline | ElementId::Polygon | ElementId::Path => {
            if let Some(path) = super::shapes::convert(node, state) {
                convert_path(node, path, state, cache, parent);
            }
        }
...
pub(crate) fn convert_group(
    ...
    let opacity = if state.parent_clip_path.is_none() {
        node.attribute::<Opacity>(AttributeId::Opacity)
            .unwrap_or(Opacity::ONE)
    } else {
        Opacity::ONE
    };

    let transform = node.resolve_transform(AttributeId::Transform, state);
    let blend_mode: BlendMode = node.attribute(AttributeId::MixBlendMode).unwrap_or_default();
    let isolation: Isolation = node.attribute(AttributeId::Isolation).unwrap_or_default();
    let isolate = isolation == Isolation::Isolate;

    // Nodes generated by markers must not have an ID. Otherwise we would have duplicates.
    let is_g_or_use = matches!(node.tag_name(), Some(ElementId::G) | Some(ElementId::Use));
    ...
    if let Some(link) = node.attribute::<SvgNode>(AttributeId::ClipPath) {
    ...
    if state.parent_clip_path.is_none() {
        if let Some(link) = node.attribute::<SvgNode>(AttributeId::Mask) {
    ...
    let filters = {
        ...
        if state.parent_clip_path.is_none() {
            if node.attribute(AttributeId::Filter) == Some("none") {
                // Do nothing.
            } else if node.has_attribute(AttributeId::Filter) {
    ...
    let grouping_required = opacity.get().approx_ne_ulps(&1.0, 4)
        || clip_path.is_some()
        || mask.is_some()
        || !filters.is_empty()
...
impl SvgNode<'_, '_> {
    pub(crate) fn resolve_transform(&self, transform_aid: AttributeId, state: &State) -> Transform {
        let mut transform: Transform = self.attribute(transform_aid).unwrap_or_default();
        let transform_origin: Option<TransformOrigin> = self.attribute(AttributeId::TransformOrigin);
