...
use super::svgtree::{AttributeId, FromValue, SvgNode};
...
pub(crate) fn resolve_fill(
    ...
    if state.parent_clip_path.is_some() {
        ...
        return Some(Fill {
            ...
            opacity: Opacity::ONE,
            rule: node.find_attribute(AttributeId::ClipRule).unwrap_or_default(),
    ...
    let (paint, context_element) =
        if let Some(n) = node.ancestors().find(|n| n.has_attribute(AttributeId::Fill)) {
            let value: &str = n.attribute(AttributeId::Fill)?;
            convert_paint(
                node,
                value,
                AttributeId::Fill,
    ...
    let fill_opacity = node
        .find_attribute::<Opacity>(AttributeId::FillOpacity)
        .unwrap_or(Opacity::ONE);

    Some(Fill {
        paint,
        opacity: sub_opacity * fill_opacity,
        rule: node.find_attribute(AttributeId::FillRule).unwrap_or_default(),
        context_element,
    })
}
...
pub(crate) fn resolve_stroke(
    ...
    let mut sub_opacity = Opacity::ONE;
    let (paint, context_element) =
        if let Some(n) = node.ancestors().find(|n| n.has_attribute(AttributeId::Stroke)) {
            let value: &str = n.attribute(AttributeId::Stroke)?;
    ...
    let width = node.resolve_valid_length(AttributeId::StrokeWidth, state, 1.0)?;

    // Must be bigger than 1.
    let miterlimit = node.find_attribute(AttributeId::StrokeMiterlimit).unwrap_or(4.0);
    ...
    let stroke_opacity = node
        .find_attribute::<Opacity>(AttributeId::StrokeOpacity)
        .unwrap_or(Opacity::ONE);

    let stroke = Stroke {
        paint,
        dasharray: conv_dasharray(node, state),
        dashoffset: node.resolve_length(AttributeId::StrokeDashoffset, state, 0.0),
        miterlimit,
        opacity: sub_opacity * stroke_opacity,
        width,
        linecap: node.find_attribute(AttributeId::StrokeLinecap).unwrap_or_default(),
        linejoin: node.find_attribute(AttributeId::StrokeLinejoin).unwrap_or_default(),
        context_element,
    };
...
fn convert_paint(
    ...
    match paint {
        ...
        svgtypes::Paint::CurrentColor => {
            let svg_color: Option<svgtypes::Color> = node
...
fn conv_dasharray(node: SvgNode, state: &converter::State) -> Option<Vec<f32>> {
    let node = node
        .ancestors()
        .find(|n| n.has_attribute(AttributeId::StrokeDasharray))?;
    let list = super::units::convert_list(node, AttributeId::StrokeDasharray, state)?;
