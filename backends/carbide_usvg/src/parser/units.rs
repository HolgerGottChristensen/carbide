        ...
        Unit::Percent => {
            if object_units == Units::ObjectBoundingBox {
                ...
                match aid {
                    ...
                    | AttributeId::Dx
                    | AttributeId::Fx
                    | AttributeId::MarkerWidth
                    | AttributeId::RefX
                    | AttributeId::Rx
                    | AttributeId::Width
                    | AttributeId::X
                    | AttributeId::X1
                    | AttributeId::X2 => convert_percent(length, view_box.width()),
                    AttributeId::Cy
                    | AttributeId::Dy
                    | AttributeId::Fy
                    | AttributeId::Height
                    | AttributeId::MarkerHeight
                    | AttributeId::RefY
                    | AttributeId::Ry
                    | AttributeId::Y
                    | AttributeId::Y1
                    | AttributeId::Y2 => convert_percent(length, view_box.height()),
                    _ => {
...
#[inline(never)]
pub(crate) fn convert_list(node: SvgNode, aid: AttributeId, state: &converter::State) -> Option<Vec<f32>> {
...
pub(crate) fn resolve_font_size(node: SvgNode, state: &converter::State) -> f32 {
    ...
    for n in nodes.iter().rev().skip(1) {
        // skip Root
        if let Some(length) = n.try_attribute::<Length>(AttributeId::FontSize) {
            let dpi = state.opt.dpi;
            let n = length.number as f32;
        ...
        } else if let Some(name) = n.attribute(AttributeId::FontSize) {
