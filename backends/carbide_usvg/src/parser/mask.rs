...
pub(crate) fn convert(
    ...
    if node.tag_name() != Some(ElementId::Mask) {
        return None;
    }

    let units = node
        .attribute(AttributeId::MaskUnits)
        .unwrap_or(Units::ObjectBoundingBox);

    let content_units = node
        .attribute(AttributeId::MaskContentUnits)
    ...
    let rect = NonZeroRect::from_xywh(
        node.convert_length(AttributeId::X, units, state, Length::new(-10.0, Unit::Percent)),
        node.convert_length(AttributeId::Y, units, state, Length::new(-10.0, Unit::Percent)),
        node.convert_length(AttributeId::Width, units, state, Length::new(120.0, Unit::Percent)),
        node.convert_length(AttributeId::Height, units, state, Length::new(120.0, Unit::Percent)),
    );
    let mut rect =
    ...
    let kind = if node.attribute(AttributeId::MaskType) == Some("alpha") {
        MaskType::Alpha
    } else {
        MaskType::Luminance
    };
