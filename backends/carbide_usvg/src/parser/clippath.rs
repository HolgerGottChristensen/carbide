...
pub(crate) fn convert(
    ...
    if node.tag_name() != Some(ElementId::ClipPath) {
        return None;
    }
    ...
    let units = node
        .attribute(AttributeId::ClipPathUnits)
...
fn resolve_clip_path_transform(node: SvgNode, state: &converter::State) -> Option<Transform> {
    ...
    let value: &str = match node.attribute(AttributeId::Transform) {
        Some(v) => v,
    ...
    let ts = match svgtypes::Transform::from_str(value) {
        Ok(v) => v,
        Err(_) => {
            log::warn!("Failed to parse {} value: '{}'.", AttributeId::Transform, value);
            return None;
        }
    };
    ...
    if ts.is_valid() {
        Some(node.resolve_transform(AttributeId::Transform, state))
    } else {
        None
    }
}
