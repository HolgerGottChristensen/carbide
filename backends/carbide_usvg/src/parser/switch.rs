...
pub(crate) fn is_condition_passed(node: SvgNode, opt: &Options) -> bool {
    ...
    if node.has_attribute(AttributeId::RequiredExtensions) {
        return false;
    }
    ...
    if let Some(features) = node.attribute::<&str>(AttributeId::RequiredFeatures) {
...
fn is_valid_sys_lang(node: SvgNode, opt: &Options) -> bool {
    ...
    if let Some(langs) = node.attribute::<&str>(AttributeId::SystemLanguage) {
