...
use super::svgtree::{AttributeId, ElementId, FromValue, SvgNode};

impl<'a, 'input: 'a> FromValue<'a, 'input> for filter::ColorInterpolation {
    fn parse(_: SvgNode, _: AttributeId, value: &str) -> Option<Self> {
        match value {
...
pub(crate) fn convert(
    ...
    let value = match node.attribute::<&str>(AttributeId::Filter) {
        Some(v) => v,
...
fn convert_url(
    ...
    let units = convert_units(node, AttributeId::FilterUnits, Units::ObjectBoundingBox);
    let primitive_units = convert_units(node, AttributeId::PrimitiveUnits, Units::UserSpaceOnUse);
...
fn find_filter_with_primitives<'a>(node: SvgNode<'a, 'a>) -> Option<SvgNode<'a, 'a>> {
    for link in node.href_iter() {
        if link.tag_name() != Some(ElementId::Filter) {
            log::warn!(
...
fn collect_children(
    ...
    for child in filter.children() {
        ...
        let kind =
            match tag_name {
                ElementId::FeDropShadow => convert_drop_shadow(child, scale, &primitives),
                ElementId::FeGaussianBlur => convert_gaussian_blur(child, scale, &primitives),
                ElementId::FeOffset => convert_offset(child, scale, &primitives),
                ElementId::FeBlend => convert_blend(child, &primitives),
                ElementId::FeFlood => convert_flood(child),
                ElementId::FeComposite => convert_composite(child, &primitives),
                ElementId::FeMerge => convert_merge(child, &primitives),
                ElementId::FeTile => convert_tile(child, &primitives),
                ElementId::FeImage => convert_image(child, filter_subregion, state, cache),
                ElementId::FeComponentTransfer => convert_component_transfer(child, &primitives),
                ElementId::FeColorMatrix => convert_color_matrix(child, &primitives),
                ElementId::FeConvolveMatrix => convert_convolve_matrix(child, &primitives)
                    .unwrap_or_else(create_dummy_primitive),
                ElementId::FeMorphology => convert_morphology(child, scale, &primitives),
                ElementId::FeDisplacementMap => convert_displacement_map(child, scale, &primitives),
                ElementId::FeTurbulence => convert_turbulence(child),
                ElementId::FeDiffuseLighting => convert_diffuse_lighting(child, &primitives)
                    .unwrap_or_else(create_dummy_primitive),
                ElementId::FeSpecularLighting => convert_specular_lighting(child, &primitives)
                    .unwrap_or_else(create_dummy_primitive),
                tag_name => {
        ...
        let color_interpolation = child
            .find_attribute(AttributeId::ColorInterpolationFilters)
            .unwrap_or_default();
...
fn resolve_primitive_region(
    ...
    let x = fe.try_convert_length(AttributeId::X, units, state);
    let y = fe.try_convert_length(AttributeId::Y, units, state);
    let width = fe.try_convert_length(AttributeId::Width, units, state);
    let height = fe.try_convert_length(AttributeId::Height, units, state);

    let region = match kind {
        ElementId::FeFlood | ElementId::FeImage => {
...
#[inline(never)]
fn resolve_input(node: SvgNode, aid: AttributeId, primitives: &[Primitive]) -> Input {
...
fn gen_result(node: SvgNode, results: &mut FilterResults) -> String {
    match node.attribute::<&str>(AttributeId::Result) {
        Some(s) => {
...
fn convert_blend(fe: SvgNode, primitives: &[Primitive]) -> Kind {
    let mode = fe.attribute(AttributeId::Mode).unwrap_or_default();
    let input1 = resolve_input(fe, AttributeId::In, primitives);
    let input2 = resolve_input(fe, AttributeId::In2, primitives);
    Kind::Blend(Blend {
        mode,
        input1,
        input2,
...
fn convert_color_matrix(fe: SvgNode, primitives: &[Primitive]) -> Kind {
    ...
    Kind::ColorMatrix(ColorMatrix {
        input: resolve_input(fe, AttributeId::In, primitives),
        kind,
    })
}

fn convert_color_matrix_kind(fe: SvgNode) -> Option<ColorMatrixKind> {
    match fe.attribute(AttributeId::Type) {
        Some("saturate") => {
            if let Some(list) = fe.attribute::<Vec<f32>>(AttributeId::Values) {
                if !list.is_empty() {
    ...
    for child in fe.children().filter(|n| n.is_element()) {
        if let Some(func) = convert_transfer_function(child) {
            match child.tag_name().unwrap() {
                ElementId::FeFuncR => kind.func_r = func,
                ElementId::FeFuncG => kind.func_g = func,
                ElementId::FeFuncB => kind.func_b = func,
                ElementId::FeFuncA => kind.func_a = func,
                _ => {}
            }
        }
    }
...
fn convert_transfer_function(node: SvgNode) -> Option<TransferFunction> {
    match node.attribute(AttributeId::Type)? {
        "identity" => Some(TransferFunction::Identity),
        "table" => match node.attribute::<Vec<f32>>(AttributeId::TableValues) {
            Some(values) => Some(TransferFunction::Table(values)),
            None => Some(TransferFunction::Table(Vec::new())),
        },
        "discrete" => match node.attribute::<Vec<f32>>(AttributeId::TableValues) {
        ...
        "linear" => Some(TransferFunction::Linear {
            slope: node.attribute(AttributeId::Slope).unwrap_or(1.0),
            intercept: node.attribute(AttributeId::Intercept).unwrap_or(0.0),
        }),
        "gamma" => Some(TransferFunction::Gamma {
            amplitude: node.attribute(AttributeId::Amplitude).unwrap_or(1.0),
            exponent: node.attribute(AttributeId::Exponent).unwrap_or(1.0),
            offset: node.attribute(AttributeId::Offset).unwrap_or(0.0),
        }),
        _ => None,
    }
}
...
fn convert_composite(fe: SvgNode, primitives: &[Primitive]) -> Kind {
    let operator = match fe.attribute(AttributeId::Operator).unwrap_or("over") {
        ...
        "arithmetic" => CompositeOperator::Arithmetic {
            k1: fe.attribute(AttributeId::K1).unwrap_or(0.0),
            k2: fe.attribute(AttributeId::K2).unwrap_or(0.0),
            k3: fe.attribute(AttributeId::K3).unwrap_or(0.0),
            k4: fe.attribute(AttributeId::K4).unwrap_or(0.0),
        },
...
fn convert_convolve_matrix(fe: SvgNode, primitives: &[Primitive]) -> Option<Kind> {
    ...
    let mut order_x = 3;
    let mut order_y = 3;
    if let Some(value) = fe.attribute::<&str>(AttributeId::Order) {
    ...
    if let Some(list) = fe.attribute::<Vec<f32>>(AttributeId::KernelMatrix) {
    ...
    let divisor = fe.attribute(AttributeId::Divisor).unwrap_or(kernel_sum);
    if divisor.approx_zero_ulps(4) {
        return None;
    }

    let bias = fe.attribute(AttributeId::Bias).unwrap_or(0.0);

    let target_x = parse_target(fe.attribute(AttributeId::TargetX), order_x)?;
    let target_y = parse_target(fe.attribute(AttributeId::TargetY), order_y)?;

    let kernel_matrix = ConvolveMatrixData::new(target_x, target_y, order_x, order_y, matrix)?;

    let edge_mode = match fe.attribute(AttributeId::EdgeMode).unwrap_or("duplicate") {
        "none" => EdgeMode::None,
        "wrap" => EdgeMode::Wrap,
        _ => EdgeMode::Duplicate,
    };

    let preserve_alpha = fe.attribute(AttributeId::PreserveAlpha).unwrap_or("false") == "true";
...
fn convert_displacement_map(fe: SvgNode, scale: Size, primitives: &[Primitive]) -> Kind {
    ...
    Kind::DisplacementMap(DisplacementMap {
        input1: resolve_input(fe, AttributeId::In, primitives),
        input2: resolve_input(fe, AttributeId::In2, primitives),
        scale: fe.attribute(AttributeId::Scale).unwrap_or(0.0) * scale,
        x_channel_selector: parse_channel(AttributeId::XChannelSelector),
        y_channel_selector: parse_channel(AttributeId::YChannelSelector),
    })
}
...
fn convert_drop_shadow(fe: SvgNode, scale: Size, primitives: &[Primitive]) -> Kind {
    ...
    let (color, opacity) = fe
        .attribute(AttributeId::FloodColor)
        .unwrap_or_else(svgtypes::Color::black)
        .split_alpha();

    let flood_opacity = fe
        .attribute::<Opacity>(AttributeId::FloodOpacity)
        .unwrap_or(Opacity::ONE);

    Kind::DropShadow(DropShadow {
        input: resolve_input(fe, AttributeId::In, primitives),
        dx: fe.attribute(AttributeId::Dx).unwrap_or(2.0) * scale.width(),
        dy: fe.attribute(AttributeId::Dy).unwrap_or(2.0) * scale.height(),
        std_dev_x,
        std_dev_y,
        color,
...
fn convert_std_dev_attr(fe: SvgNode, scale: Size, default: &str) -> (PositiveF32, PositiveF32) {
    let text = fe.attribute(AttributeId::StdDeviation).unwrap_or(default);
...
fn convert_image_inner(
    ...
    let rendering_mode = fe
        .find_attribute(AttributeId::ImageRendering)
        .unwrap_or(state.opt.image_rendering);

    if let Some(node) = fe.try_attribute::<SvgNode>(AttributeId::Href) {
    ...
    let href = fe.try_attribute(AttributeId::Href).log_none(|| {
    ...
    let aspect: AspectRatio = fe.attribute(AttributeId::PreserveAspectRatio).unwrap_or_default();
...
fn convert_diffuse_lighting(fe: SvgNode, primitives: &[Primitive]) -> Option<Kind> {
    ...
    Some(Kind::DiffuseLighting(DiffuseLighting {
        input: resolve_input(fe, AttributeId::In, primitives),
        surface_scale: fe.attribute(AttributeId::SurfaceScale).unwrap_or(1.0),
        diffuse_constant: fe.attribute(AttributeId::DiffuseConstant).unwrap_or(1.0),
...
fn convert_specular_lighting(fe: SvgNode, primitives: &[Primitive]) -> Option<Kind> {
    ...
    let specular_exponent = fe.attribute(AttributeId::SpecularExponent).unwrap_or(1.0);
    ...
    Some(Kind::SpecularLighting(SpecularLighting {
        input: resolve_input(fe, AttributeId::In, primitives),
        surface_scale: fe.attribute(AttributeId::SurfaceScale).unwrap_or(1.0),
        specular_constant: fe.attribute(AttributeId::SpecularConstant).unwrap_or(1.0),
        specular_exponent,
        lighting_color: convert_lighting_color(fe),
        light_source,
    }))
...
fn convert_lighting_color(node: SvgNode) -> Color {
    ...
    match node.attribute(AttributeId::LightingColor) {
        Some("currentColor") => {
            node.find_attribute(AttributeId::Color)
...
fn convert_light_source(parent: SvgNode) -> Option<LightSource> {
    let child = parent.children().find(|n| {
        matches!(
            n.tag_name(),
            Some(ElementId::FeDistantLight) | Some(ElementId::FePointLight) | Some(ElementId::FeSpotLight)
        )
    })?;

    match child.tag_name() {
        Some(ElementId::FeDistantLight) => Some(LightSource::DistantLight(DistantLight {
            azimuth: child.attribute(AttributeId::Azimuth).unwrap_or(0.0),
            elevation: child.attribute(AttributeId::Elevation).unwrap_or(0.0),
        })),
        Some(ElementId::FePointLight) => Some(LightSource::PointLight(PointLight {
            x: child.attribute(AttributeId::X).unwrap_or(0.0),
            y: child.attribute(AttributeId::Y).unwrap_or(0.0),
            z: child.attribute(AttributeId::Z).unwrap_or(0.0),
        })),
        Some(ElementId::FeSpotLight) => {
            let specular_exponent = child.attribute(AttributeId::SpecularExponent).unwrap_or(1.0);
            ...
            Some(LightSource::SpotLight(SpotLight {
                ...
                y: child.attribute(AttributeId::Y).unwrap_or(0.0),
                z: child.attribute(AttributeId::Z).unwrap_or(0.0),
                points_at_x: child.attribute(AttributeId::PointsAtX).unwrap_or(0.0),
                points_at_y: child.attribute(AttributeId::PointsAtY).unwrap_or(0.0),
                points_at_z: child.attribute(AttributeId::PointsAtZ).unwrap_or(0.0),
                specular_exponent,
                limiting_cone_angle: child.attribute(AttributeId::LimitingConeAngle),
            }))
        }
        _ => None,
    }
...
fn convert_merge(fe: SvgNode, primitives: &[Primitive]) -> Kind {
    ...
    for child in fe.children() {
        inputs.push(resolve_input(child, AttributeId::In, primitives));
    }
...
fn convert_morphology(fe: SvgNode, scale: Size, primitives: &[Primitive]) -> Kind {
    let operator = match fe.attribute(AttributeId::Operator).unwrap_or("erode") {
    ...
    if let Some(list) = fe.attribute::<Vec<f32>>(AttributeId::Radius) {
        let mut rx = 0.0;
        let mut ry = 0.0;
        if list.len() == 2 {
            rx = list[0];
...
fn convert_offset(fe: SvgNode, scale: Size, primitives: &[Primitive]) -> Kind {
    Kind::Offset(Offset {
        input: resolve_input(fe, AttributeId::In, primitives),
        dx: fe.attribute(AttributeId::Dx).unwrap_or(0.0) * scale.width(),
        dy: fe.attribute(AttributeId::Dy).unwrap_or(0.0) * scale.height(),
    })
}
...
fn convert_turbulence(fe: SvgNode) -> Kind {
    ...
    if let Some(list) = fe.attribute::<Vec<f32>>(AttributeId::BaseFrequency) {
        let mut x = 0.0;
        let mut y = 0.0;
        if list.len() == 2 {
            x = list[0];
    ...
    let mut num_octaves = fe.attribute(AttributeId::NumOctaves).unwrap_or(1.0);
    if num_octaves.is_sign_negative() {
        num_octaves = 0.0;
    }

    let kind = match fe.attribute(AttributeId::Type).unwrap_or("turbulence") {
    ...
    Kind::Turbulence(Turbulence {
        ...
        seed: fe.attribute::<f32>(AttributeId::Seed).unwrap_or(0.0).trunc() as i32,
        stitch_tiles: fe.attribute(AttributeId::StitchTiles) == Some("stitch"),
        kind,
    })
}
...
fn convert_drop_shadow_function(
    ...
    Kind::DropShadow(DropShadow {
        input: Input::SourceGraphic,
        dx: super::units::convert_user_length(dx, node, AttributeId::Dx, state),
        dy: super::units::convert_user_length(dy, node, AttributeId::Dy, state),
        std_dev_x: std_dev,
        std_dev_y: std_dev,
        color,
        opacity,
