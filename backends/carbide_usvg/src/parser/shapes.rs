...
pub(crate) fn convert(node: SvgNode, state: &converter::State) -> Option<Arc<Path>> {
    match node.tag_name()? {
        ElementId::Rect => convert_rect(node, state),
        ElementId::Circle => convert_circle(node, state),
        ElementId::Ellipse => convert_ellipse(node, state),
        ElementId::Line => convert_line(node, state),
        ElementId::Polyline => convert_polyline(node),
        ElementId::Polygon => convert_polygon(node),
        ElementId::Path => convert_path(node),
        _ => None,
    }
}

pub(crate) fn convert_path(node: SvgNode) -> Option<Arc<Path>> {
    let value: &str = node.attribute(AttributeId::D)?;
...
fn convert_rect(node: SvgNode, state: &converter::State) -> Option<Arc<Path>> {
    ...
    let width = node.convert_user_length(AttributeId::Width, state, Length::zero());
    let height = node.convert_user_length(AttributeId::Height, state, Length::zero());
...
fn resolve_rx_ry(node: SvgNode, state: &converter::State) -> (f32, f32) {
    let mut rx_opt = node.attribute::<Length>(AttributeId::Rx);
    let mut ry_opt = node.attribute::<Length>(AttributeId::Ry);
    ...
    match (rx_opt, ry_opt) {
        (None, None) => (0.0, 0.0),
        (Some(rx), None) => {
            let rx = units::convert_user_length(rx, node, AttributeId::Rx, state);
            (rx, rx)
        }
        (None, Some(ry)) => {
            let ry = units::convert_user_length(ry, node, AttributeId::Ry, state);
            (ry, ry)
        }
        (Some(rx), Some(ry)) => {
            let rx = units::convert_user_length(rx, node, AttributeId::Rx, state);
...
fn convert_line(node: SvgNode, state: &converter::State) -> Option<Arc<Path>> {
    let x1 = node.convert_user_length(AttributeId::X1, state, Length::zero());
    let y1 = node.convert_user_length(AttributeId::Y1, state, Length::zero());
    let x2 = node.convert_user_length(AttributeId::X2, state, Length::zero());
    let y2 = node.convert_user_length(AttributeId::Y2, state, Length::zero());
...
fn points_to_path(node: SvgNode, eid: &str) -> Option<tiny_skia_path::PathBuilder> {
    ...
    let mut builder = tiny_skia_path::PathBuilder::new();
    match node.attribute::<&str>(AttributeId::Points) {
        Some(text) => {
...
fn convert_circle(node: SvgNode, state: &converter::State) -> Option<Arc<Path>> {
    let cx = node.convert_user_length(AttributeId::Cx, state, Length::zero());
    let cy = node.convert_user_length(AttributeId::Cy, state, Length::zero());
    let r = node.convert_user_length(AttributeId::R, state, Length::zero());
