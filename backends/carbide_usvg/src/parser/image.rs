...
use super::svgtree::{AttributeId, SvgNode};
...
pub(crate) fn convert(
    ...
    let href = node
        .try_attribute(AttributeId::Href)
    ...
    match (
        node.attribute::<Length>(AttributeId::Width),
        node.attribute::<Length>(AttributeId::Height),
    ) {
        (Some(_), None) => {
    ...
    let aspect: AspectRatio = node.attribute(AttributeId::PreserveAspectRatio).unwrap_or_default();
