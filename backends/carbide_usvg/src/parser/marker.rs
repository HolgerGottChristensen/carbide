...
pub(crate) fn is_valid(node: SvgNode) -> bool {
    ...
    if node
        .ancestors()
        .any(|n| n.tag_name() == Some(ElementId::ClipPath))
    {
        return false;
    }

    let start = node.find_attribute::<SvgNode>(AttributeId::MarkerStart);
    let mid = node.find_attribute::<SvgNode>(AttributeId::MarkerMid);
    let end = node.find_attribute::<SvgNode>(AttributeId::MarkerEnd);
...
pub(crate) fn convert(
    ...
    let list = [
        (AttributeId::MarkerStart, MarkerKind::Start),
        (AttributeId::MarkerMid, MarkerKind::Middle),
        (AttributeId::MarkerEnd, MarkerKind::End),
    ];

    for (aid, kind) in &list {
        let mut marker = None;
        if let Some(link) = node.find_attribute::<SvgNode>(*aid) {
            if link.tag_name() == Some(ElementId::Marker) {
                marker = Some(link);
            }
        }
...
fn resolve(
    ...
    let has_overflow = {
        let overflow = marker_node.attribute(AttributeId::Overflow);
...
fn stroke_scale(
    ...
    match marker_node.attribute(AttributeId::MarkerUnits) {
        Some("userSpaceOnUse") => NonZeroPositiveF32::new(1.0),
        _ => path_node.resolve_valid_length(AttributeId::StrokeWidth, state, 1.0),
    }
}
...
fn convert_rect(node: SvgNode, state: &converter::State) -> Option<NonZeroRect> {
    NonZeroRect::from_xywh(
        node.convert_user_length(AttributeId::RefX, state, Length::zero()),
        node.convert_user_length(AttributeId::RefY, state, Length::zero()),
        node.convert_user_length(AttributeId::MarkerWidth, state, Length::new_number(3.0)),
        node.convert_user_length(AttributeId::MarkerHeight, state, Length::new_number(3.0)),
    )
}

fn convert_orientation(node: SvgNode) -> MarkerOrientation {
    match node.attribute(AttributeId::Orient) {
        Some("auto") => MarkerOrientation::Auto,
        Some("auto-start-reverse") => MarkerOrientation::AutoStartReverse,
        _ => match node.attribute::<svgtypes::Angle>(AttributeId::Orient) {
