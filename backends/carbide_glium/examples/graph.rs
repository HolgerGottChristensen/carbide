...
use carbide_core::widget::graph::{node, Event, EdgeEvent, Node, NodeEvent, NodeSocket};
use glium::Surface;
...
fn main() {
    ...
    let window = glium::glutin::WindowBuilder::new()
        .with_title("carbide Graph Widget")
...
fn set_widgets(ui: &mut carbide_core::UiCell, ids: &Ids, graph: &mut MyGraph, layout: &mut Layout) {
    ...
    let session = {
        ...
        widget::Graph::new(node_indices, edges, layout)
            .background_color(carbide_core::color::rgb(0.31, 0.33, 0.35))
            .wh_of(ui.window)
            .middle_of(ui.window)
            .set(ids.graph, ui)
    };
    ...
    for node in session.nodes() {
        ...
        let widget = Node::new(button)
            .inputs(inputs)
            .outputs(outputs)
            //.socket_color(carbide_core::color::LIGHT_RED)
            .w_h(100.0, 60.0);
