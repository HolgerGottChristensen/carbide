//! A backend for converting src events to carbide's `Input` type.

use carbide_core::{Point, Scalar, input, event};
pub use piston_input::{GenericEvent, UpdateEvent};

/// Converts any `GenericEvent` to an `Input` event for carbide.
///
    ...
    where E: GenericEvent,
{
    ...
    if let Some(xy) = event.mouse_scroll_args() {
        // Invert the scrolling of the *y* axis as *y* is up in carbide.
        let (x, y) = (xy[0], -xy[1]);
