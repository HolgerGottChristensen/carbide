...
pub fn main() {
    ...
    let (mut glyph_cache, mut text_texture_cache) = {
        ...
        let cache = carbide_core::text::GlyphCache::builder()
    ...
    while let Some(event) = window.next() {
        ...
        // Convert the src event to a carbide event.
        let size = window.size();
        let (win_w, win_h) = (size.width as carbide_core::Scalar, size.height as carbide_core::Scalar);
        if let Some(e) = carbide_piston::event::convert(event.clone(), win_w, win_h) {
            ui.handle_event(e);
        }
        ...
        window.draw_2d(&event, |context, graphics, device| {
            if let Some((primitives, cprims)) = ui.draw_if_changed() {
                ...
                let cache_queued_glyphs = |_graphics: &mut G2d,
                                           ...
                                           rect: carbide_core::text::rt::Rect<u32>,
                                           data: &[u8]|
                    {
                ...
                // Draw the carbide `render::Primitives`.
                carbide_piston::draw::primitives(primitives,
                                                context,
                                                graphics,
