//! [![github]](https://github.com/dtolnay/carbide_syn)&ensp;[![crates-io]](https://crates.io/crates/carbide_syn)&ensp;[![docs-rs]](crate)
//!
...
//! <br>
//!
//! carbide_syn is a parsing library for parsing a stream of Rust tokens into a syntax
//! tree of Rust source code.
//!
//! Currently this library is geared toward use in Rust procedural macros, but
//! contains some APIs that may be useful more generally.
//!
//! - **Data structures** — carbide_syn provides a complete syntax tree that can
//!   represent any valid Rust source code. The syntax tree is rooted at
//!   [`carbide_syn::File`] which represents a full source file, but there are other
//!   entry points that may be useful to procedural macros including
//!   [`carbide_syn::Item`], [`carbide_syn::Expr`] and [`carbide_syn::Type`].
//!
//! - **Derives** — Of particular interest to derive macros is
//!   [`carbide_syn::DeriveInput`] which is any of the three legal input items to a
//!   derive macro. An example below shows using this type in a library that can
//!   derive implementations of a user-defined trait.
//!
//! - **Parsing** — Parsing in carbide_syn is built around [parser functions] with the
//!   signature `fn(ParseStream) -> Result<T>`. Every syntax tree node defined
//!   by carbide_syn is individually parsable and may be used as a building block for
//!   custom syntaxes, or you may dream up your own brand new syntax without
//!   involving any of our syntax tree types.
//!
//! - **Location information** — Every token parsed by carbide_syn is associated with a
...
//!
//! [`carbide_syn::File`]: File
//! [`carbide_syn::Item`]: Item
//! [`carbide_syn::Expr`]: Expr
//! [`carbide_syn::Type`]: Type
//! [`carbide_syn::DeriveInput`]: DeriveInput
...
//!
//! The canonical derive macro using carbide_syn looks like this. We write an ordinary
...
//! use quote::quote;
//! use carbide_syn::{parse_macro_input, DeriveInput};
//!
...
//!
//! [`heapsize`]: https://github.com/dtolnay/carbide_syn/tree/master/examples/heapsize
//!
//! ```
//! pub trait HeapSize {
...
//! carbide_syn are able to trigger errors that directly pinpoint the source of the
//! problem.
//!
//! ```text
...
//! using carbide_syn's parsing API.
//!
//! [`lazy-static`]: https://github.com/dtolnay/carbide_syn/tree/master/examples/lazy-static
//!
...
//! # Optional features
//!
//! carbide_syn puts a lot of functionality behind optional features in order to
...
// carbide_syn types in rustdoc of other crates get linked to here.
#![doc(html_root_url = "https://docs.rs/carbide_syn/2.0.38")]
...
mod gen {
    ...
    ///
    /// ```
    /// # use carbide_syn::{Attribute, BinOp, Expr, ExprBinary};
    /// #
    /// pub trait Fold {
    ///     /* ... */
    ///
    ...
    /// // [dependencies]
    /// // quote = "1.0"
    /// // carbide_syn = { version = "2.0", features = ["fold", "full"] }
    ///
    /// use quote::quote;
    /// use carbide_syn::fold::{fold_expr, Fold};
    /// use carbide_syn::{token, Expr, ExprParen};
    ///
    ...
    ///     let expr: Expr = carbide_syn::parse2(code).unwrap();
    ...
    /// // [dependencies]
    /// // quote = "1.0"
    /// // carbide_syn = { version = "2.0", features = ["full", "visit"] }
    ///
    /// use quote::quote;
    /// use carbide_syn::visit::{self, Visit};
    /// use carbide_syn::{File, ItemFn};
    ///
    /// struct FnVisitor;
    ///
    ...
    ///     };
    ///
    ///     let syntax_tree: File = carbide_syn::parse2(code).unwrap();
    ...
    /// // [dependencies]
    /// // quote = "1.0"
    /// // carbide_syn = { version = "2.0", features = ["full", "visit-mut"] }
    ///
    /// use quote::quote;
    /// use carbide_syn::visit_mut::{self, VisitMut};
    /// use carbide_syn::{parse_quote, Expr, File, Lit, LitInt};
    ///
    ...
    ///                     let unsuffixed: LitInt = carbide_syn::parse_str(digits).unwrap();
    ...
    ///     };
    ///
    ///     let mut syntax_tree: File = carbide_syn::parse2(code).unwrap();
...
/// `proc_macro2::TokenStream`, use [`carbide_syn::parse2`] instead.
///
/// [`carbide_syn::parse2`]: parse2
///
/// # Examples
///
/// ```
...
///     let ast: DeriveInput = carbide_syn::parse(input).unwrap();
///
///     // Build the output, possibly using quasi-quotation
///     let expanded = quote! {
///         /* ... */
...
/// when the input comes from a node of the carbide_syn syntax tree, for example the
/// body tokens of a [`Macro`] node. When in a procedural macro parsing the
/// `proc_macro::TokenStream` provided by the compiler, use [`carbide_syn::parse`]
/// instead.
///
/// [`carbide_syn::parse`]: parse()
#[cfg(feature = "parsing")]
#[cfg_attr(doc_cfg, doc(cfg(feature = "parsing")))]
...
///
/// ```
/// use carbide_syn::{Expr, Result};
///
/// fn run() -> Result<()> {
///     let code = "assert_eq!(u8::max_value(), 255)";
///     let expr = carbide_syn::parse_str::<Expr>(code)?;
///     println!("{:#?}", expr);
///     Ok(())
/// }
/// #
...
///
/// This is different from `carbide_syn::parse_str::<File>(content)` in two ways:
///
...
///
///     let ast = carbide_syn::parse_file(&content)?;
