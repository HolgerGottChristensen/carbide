...
/// use carbide_syn::{parse_macro_input, LitStr, Path};
///
/// # const IGNORE: &str = stringify! {
...
///     let tea_parser = carbide_syn::meta::parser(|meta| {
...
/// ```
///
/// The `carbide_syn::meta` library will take care of dealing with the commas including
...
/// #
/// use proc_macro::TokenStream;
/// use carbide_syn::meta::ParseNestedMeta;
/// use carbide_syn::parse::{Parser, Result};
/// use carbide_syn::{parse_macro_input, LitStr, Path};
///
/// # const IGNORE: &str = stringify! {
/// #[proc_macro_attribute]
...
///     let tea_parser = carbide_syn::meta::parser(|meta| attrs.parse(meta));
///     parse_macro_input!(args with tea_parser);
///
///     /* ... */
/// #   TokenStream::new()
...
///
/// - [`carbide_syn::meta::parser`] if you are implementing a `proc_macro_attribute`
...
///
/// [`Attribute::parse_nested_meta`]: crate::Attribute::parse_nested_meta
/// [`carbide_syn::meta::parser`]: crate::meta::parser
#[non_exhaustive]
...
impl<'a> ParseNestedMeta<'a> {
    ...
    ///
    /// ```
    /// use carbide_syn::{parse_quote, Attribute, LitStr};
    ///
    ...
    ///
    /// ```
    /// use carbide_syn::{parse_quote, Attribute};
    ///
    /// let attr: Attribute = parse_quote! {
    ///     #[tea(with(sugar, milk))]
    /// };
    ...
    ///
    /// ```
    /// use carbide_syn::{parenthesized, parse_quote, Attribute, LitInt};
    ///
    /// let attr: Attribute = parse_quote! {
    ///     #[repr(align(32))]
    /// };
    ...
    ///
    /// ```
    /// # use carbide_syn::Attribute;
    /// #
    /// # fn example(attr: &Attribute) -> carbide_syn::Result<()> {
    /// attr.parse_nested_meta(|meta| {
    ///     if meta.path.is_ident("kind") {
    ///         // ...
    ///         Ok(())
    ...
    /// `carbide_syn::Error::new_spanned(&meta.path, "message...")`.
    ///
    /// ```console
    ...
    /// ```
    ///
    /// Often you may want to use `carbide_syn::Error::new_spanned` even in this
    /// situation. In the above code, that would be:
    ///
    /// ```
    /// # use carbide_syn::{Error, Expr};
    /// #
    /// # fn example(expr: Expr) -> carbide_syn::Result<()> {
    ///     match expr {
    ///         Expr::Lit(expr) => /* ... */
    /// #           unimplemented!(),
    ///         Expr::Path(expr) => /* ... */
