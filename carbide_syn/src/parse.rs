...
//!
//! Parsing in carbide_syn is built on parser functions that take in a [`ParseStream`]
...
//! use carbide_syn::{braced, parse_macro_input, token, Field, Ident, Result, Token};
...
//! ```
//!
//! # The `carbide_syn::parse*` functions
//!
//! The [`carbide_syn::parse`], [`carbide_syn::parse2`], and [`carbide_syn::parse_str`] functions serve
//! as an entry point for parsing syntax tree nodes that can be parsed in an
//! obvious default way. These functions can return any syntax tree node that
//! implements the [`Parse`] trait, which includes most types in carbide_syn.
//!
//! [`carbide_syn::parse`]: crate::parse()
//! [`carbide_syn::parse2`]: crate::parse2()
//! [`carbide_syn::parse_str`]: crate::parse_str()
//!
//! ```
//! use carbide_syn::Type;
//!
//! # fn run_parser() -> carbide_syn::Result<()> {
//! let t: Type = carbide_syn::parse_str("std::collections::HashMap<String, Value>")?;
//! #     Ok(())
//! # }
//! #
//! # run_parser().unwrap();
...
//! #
//! # use carbide_syn::punctuated::Punctuated;
//! # use carbide_syn::{PathSegment, Result, Token};
//! #
//! # fn f(tokens: proc_macro::TokenStream) -> Result<()> {
//! #
//! // Can't parse `Punctuated` without knowing whether trailing punctuation
//! // should be allowed in this context.
//! let path: Punctuated<PathSegment, Token![::]> = carbide_syn::parse(tokens)?;
//! #
//! #     Ok(())
//! # }
//! ```
...
//! #
//! use proc_macro::TokenStream;
//! use carbide_syn::parse::Parser;
//! use carbide_syn::punctuated::Punctuated;
//! use carbide_syn::{Attribute, Expr, PathSegment, Result, Token};
//!
...
/// Input to a carbide_syn parser function.
///
/// See the methods of this type under the documentation of [`ParseBuffer`]. For
/// an overview of parsing in carbide_syn, refer to the [module documentation].
///
/// [module documentation]: self
...
///
/// `ParseStream` is the input type for all parser functions in carbide_syn. They have
...
/// - One of [the `carbide_syn::parse*` functions][carbide_syn-parse]; or
/// - A method of the [`Parser`] trait.
///
/// [`parse_macro_input!`]: crate::parse_macro_input!
/// [carbide_syn-parse]: self#the-synparse-functions
...
/// use carbide_syn::Result;
/// use carbide_syn::parse::ParseStream;
///
...
impl<'a> ParseBuffer<'a> {
    ...
    ///
    /// ```
    /// use carbide_syn::{Attribute, Ident, Result, Token};
    /// use carbide_syn::parse::{Parse, ParseStream};
    ///
    ...
    ///
    /// ```
    /// use carbide_syn::{braced, token, Generics, Ident, Result, Token, TypeParamBound};
    /// use carbide_syn::parse::{Parse, ParseStream};
    /// use carbide_syn::punctuated::Punctuated;
    ///
    ...
    ///
    /// ```
    /// use carbide_syn::{Ident, ItemUnion, Macro, Result, Token};
    /// use carbide_syn::parse::{Parse, ParseStream};
    ///
    ...
    /// # use quote::quote;
    /// #
    /// use carbide_syn::{parenthesized, token, Ident, Result, Token, Type};
    /// use carbide_syn::parse::{Parse, ParseStream};
    /// use carbide_syn::punctuated::Punctuated;
    ///
    ...
    /// #     struct S(A, B);
    /// # };
    /// # carbide_syn::parse2::<TupleStruct>(input).unwrap();
    /// ```
    ///
    /// # See also
    ///
    ...
    ///
    /// ```
    /// use carbide_syn::{custom_keyword, Expr, Result, Token};
    /// use carbide_syn::parse::{Parse, ParseStream};
    /// use carbide_syn::punctuated::Punctuated;
    ///
    /// mod kw {
    ///     carbide_syn::custom_keyword!(fin);
    /// }
    ///
    ...
    ///
    /// ```
    /// use carbide_syn::{braced, token, Ident, Item, Result, Token};
    /// use carbide_syn::parse::{Parse, ParseStream};
    ///
    ...
    ///
    /// ```
    /// use carbide_syn::{ConstParam, Ident, Lifetime, LifetimeParam, Result, Token, TypeParam};
    /// use carbide_syn::parse::{Parse, ParseStream};
    ///
    ...
    ///
    /// ```
    /// use carbide_syn::{parenthesized, token, Ident, Path, Result, Token};
    /// use carbide_syn::ext::IdentExt;
    /// use carbide_syn::parse::{Parse, ParseStream};
    ///
    ...
    ///
    /// ```
    /// use carbide_syn::{Expr, Result, Token};
    /// use carbide_syn::parse::{Parse, ParseStream};
    ///
    ...
    /// widely outside of the carbide_syn codebase.
    ///
    /// # Example
    ///
    /// ```
        ...
        F: for<'c> FnOnce(StepCursor<'c, 'a>) -> Result<(R, Cursor<'c>)>,
    {
        ...
        // Within carbide_syn we solve this using `advance_step_cursor` which uses the
        // existence of a StepCursor<'c, 'a> as proof that it is safe to cast
        // from Cursor<'c> to Cursor<'a>. If needed outside of carbide_syn, it would be
...
///
/// Refer to the [module documentation] for details about parsing in carbide_syn.
///
/// [module documentation]: self
...
/// #
/// use proc_macro::TokenStream;
/// use carbide_syn::parse_macro_input;
/// use carbide_syn::parse::Nothing;
///
/// # const IGNORE: &str = stringify! {
