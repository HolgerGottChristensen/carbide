...
impl Macro {
    ...
    /// tree.
    ///
    /// This is equivalent to `carbide_syn::parse2::<T>(mac.tokens)` except that it
    /// produces a more useful span when `tokens` is empty.
    ///
    /// # Example
    ///
    /// ```
    /// use carbide_syn::{parse_quote, Expr, ExprLit, Ident, Lit, LitStr, Macro, Token};
    /// use carbide_syn::ext::IdentExt;
    /// use carbide_syn::parse::{Error, Parse, ParseStream, Result};
