...
ast_enum_of_structs! {
    /// A Rust expression.
    ///
    /// *This type is available only if carbide_syn is built with the `"derive"` or `"full"`
    /// feature, but most of the variants are not available unless "full" is enabled.*
    ///
    /// # Syntax tree enums
    ///
    /// This type is a syntax tree enum. In carbide_syn this and other syntax tree enums
    ...
    ///
    /// ```
    /// # use carbide_syn::{Expr, ExprMethodCall};
    /// #
    /// # fn example(expr: Expr) {
    ...
    ///
    /// ```
    /// # use carbide_syn::{Expr, ExprField};
    /// #
    ...
    pub enum Expr {
        ...
        /// Tokens in expression position not interpreted by carbide_syn.
...
pub(crate) mod parsing {
    ...
    fn atom_expr(input: ParseStream) -> Result<Expr> {
            ...
            || input.peek(Token![crate])
        {
            ...
            Err(input.error("unsupported expression; enable carbide_syn's features=[\"full\"]"))
        }
    }
